...
- iteration_dir: /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0022
...
2. Write iteration artifacts directly inside:
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0022
3. Required files to write:
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0022/executor_iteration_report.md
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0022/executor_next_steps.md
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0022/executor_hypothesis_screen.json
...
- iter_0021 H46 | family=topology_stability | decision=negative | direction=negative | method=Computed true zigzag H1 lifetimes on paired source->union<-target kNN complexes across 3 d
- iter_0021 H47 | family=persistent_homology | decision=promising | direction=positive | method=On seed42 across immune/lung/external_lung and source/target-disjoint splits at layers {0,
- iter_0021 H48 | family=cross_model_alignment | decision=inconclusive | direction=mixed | method=Computed scGPT (layerwise centered-cosine) and Geneformer (embedding centered-cosine) top-
- iter_0020 H43 | family=module_structure | decision=promising | direction=positive | method=On seed42 across immune/lung/external_lung and source/target-disjoint splits at layers {0,
- iter_0020 H44 | family=topology_stability | decision=promising | direction=positive | method=Using dionysus true zigzag persistence on source->union<-target kNN edge complexes for see
- iter_0020 H45 | family=intrinsic_dimensionality | decision=inconclusive | direction=mixed | method=From H38 moment features, evaluated leave-layer-out and leave-seed-out robust deltas (wins
- iter_0019 H40 | family=module_structure | decision=promising | direction=positive | method=Across immune/lung/external_lung and seeds 42/43/44 with source/target disjoint splits at 
- iter_0019 H41 | family=topology_stability | decision=inconclusive | direction=mixed | method=On seed42 across domains/splits/layers {0,3,7,11}, computed split-local neighborhood PH su
- iter_0019 H42 | family=intrinsic_dimensionality | decision=negative | direction=negative | method=Using H38 moment features, evaluated leave-one-layer-out and leave-one-seed-out prediction
...
Suggested retirement candidates by repeated negatives/inconclusive: cross_model_alignment, intrinsic_dimensionality, topology_stability

Brainstormer guidance from previous iteration (must be addressed):
Completed one brainstorming pass for `iter_0021` and wrote all requested outputs:

- `iterations/iter_0021/brainstormer_structured_feedback.md`
- `iterations/iter_0021/brainstormer_hypothesis_roadmap.md`
- `iterations/iter_0021/brainstormer_next_iteration_brief.md`
...
- Gate status: `passed_min_research_gate=true`.
- Stale triage: marked rewiring-survival lineage, GW-first correspondence, raw curvature/thinness direct scoring, coarse tier concentration, `H46` weighted-threshold zigzag, and current OOS ID delta-R2 form as `retire_now`.
- Rescue-once only: `H48` with anti-sparsity redesign, and cross-model alignment (`H33/H36`) only under utility/topology-transfer objectives.
- New portfolio: 14 hypotheses (`N212`-`N225`) spanning topology/filtration variants, manifold geometry, cross-model transfer, biological anchoring, and mechanistic signatures.
- Top 3 for immediate execution:
  1. `N212` high-probability discovery (multi-seed bifiltration robustness + utility coupling),
  2. `N213` high-risk/high-reward (directed/signed persistence pilot),
  3. `N224` cheap broad-screen (expanded motif fingerprint with anti-sparsity design).
