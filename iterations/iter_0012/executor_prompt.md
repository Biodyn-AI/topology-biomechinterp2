...
- iteration_dir: /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0012
...
2. Write iteration artifacts directly inside:
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0012
3. Required files to write:
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0012/executor_iteration_report.md
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0012/executor_next_steps.md
   - /Volumes/Crucial X6/MacBook/biomechinterp/biodyn-work/subproject_40_topology_hypothesis_screening_autoloop/iterations/iter_0012/executor_hypothesis_screen.json
...
- iter_0010 H13 | family=manifold_distance | decision=promising | direction=positive | method=Compared geodesic vs Euclidean edge-distance AUROC on immune scGPT residuals (3 seeds x 12
- iter_0010 H14 | family=topology_stability | decision=promising | direction=positive | method=Computed H1 persistence deltas vs feature-shuffle null for immune scGPT across sample-size
- iter_0010 H15 | family=cross_model_alignment | decision=neutral | direction=mixed | method=Per-domain Spearman trend test between cross-model disagreement magnitude and positive-edg
- iter_0008 H11 | family=graph_topology | decision=inconclusive | direction=inconclusive | method=Stratified geodesic H1 delta analysis on immune seed-layer rows by bridging status and k-b
- iter_0008 H12 | family=null_sensitivity | decision=negative | direction=negative | method=Compared observed geodesic H1 and distortion against two metric-matched rewiring nulls (un
- iter_0007 H09 | family=null_sensitivity | decision=negative | direction=negative | method=Ran immune full-layer source/target-disjoint screening with degree-preserving rewiring nul
- iter_0007 H10 | family=manifold_distance | decision=inconclusive | direction=mixed | method=Computed calibration-shift diagnostic per seed-layer-split as (geodesic delta - Euclidean 
- iter_0006 H07 | family=null_sensitivity | decision=negative | direction=negative | method=immune-only full-layer split screen with 3 seeds, 180 genes/test, PCA(14), H1 lifetime sum
- iter_0006 H08 | family=split_robustness | decision=neutral | direction=mixed | method=same run as H07; layer-by-layer dual-split pass matrix using Fisher p-values under feature
- iter_0005 H05 | family=null_sensitivity | decision=neutral | direction=mixed | method=Ran iterations/iter_0005/run_iter0005_screen.py in subproject40-topology over lung/immune/
- iter_0005 H06 | family=split_robustness | decision=neutral | direction=mixed | method=Within the same run, compared source_disjoint and target_disjoint split regimes for each d
- iter_0004 H03 | family=persistent_homology | decision=promising | direction=positive | method=Executed conda run -n subproject40-topology python iterations/iter_0004/run_iter0004_scree
- iter_0004 H04 | family=intrinsic_dimensionality | decision=neutral | direction=mixed | method=Within the same run, computed per seed-layer participation-ratio dimension, top-5 linearit
- iter_0003 H01 | family=persistent_homology | decision=promising | direction=positive | method=Computed ripser H1 persistence on PCA-reduced (20D) scGPT layer gene embeddings (350 genes
- iter_0003 H02 | family=cross_model_alignment | decision=inconclusive | direction=inconclusive | method=Aligned scGPT and Geneformer delta_cv_auroc vectors for centered_cosine/dot/cosine in immu
Suggested retirement candidates by repeated negatives/inconclusive: null_sensitivity

Brainstormer guidance from previous iteration (must be addressed):
Completed one brainstorming pass for `iter_0011` and wrote all requested outputs:

- `iterations/iter_0011/brainstormer_structured_feedback.md`
- `iterations/iter_0011/brainstormer_hypothesis_roadmap.md`
- `iterations/iter_0011/brainstormer_next_iteration_brief.md`

Also updated completion tracking per project rule:

- `tracking/prompt.md`

Key outcomes captured in the files:
- Stale directions triaged:
  - `retire_now`: rewiring-survival primary branch, distortion-lower-tail rescue.
  - `rescue_once_with_major_change`: bridge-conditioned rewiring explanation, coarse bin cross-model trend, mean-only intrinsic coupling.
- New portfolio: 16 testable hypotheses (`N68`–`N83`) spanning topology, manifold geometry, cross-model transfer/alignment, biological anchoring, and mechanistic motifs.
- Top 3 execution picks for next loop:
  1. High-probability discovery: `N81` (confidence-stratified biological anchoring of H16)
  2. High-risk/high-reward: `N78` (cross-model OT/Procrustes transfer)
  3. Cheap broad-screen: `N75` (local reconstruction error mechanistic screen)

No additional autonomous cycle was initiated.
